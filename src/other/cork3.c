...
if(!(f=fopen("cork.geo","w"))){printf("Could not open file 'cork.geo'\r\n"); exit(1);}
...
for(i=1;i<=2*p;i++){
  for(j=1;j<=n;j++){
    ...
    fprintf(f,"%1.4f %1.4f %1.4f\r\n",r-rij*cos(ttij),rij*sin(ttij),y[j]-0.5*s*w+s*w*ttij/(2*PI));
  }
}
...
if(!(f=fopen("cork.col","w"))){printf("Could not open file 'cork.col'\r\n"); exit(1);}
fprintf(f,"%d\r\n",4*p);
...
if(!(f=fopen("cork.ref","w"))){printf("Could not open file 'cork.ref'\r\n"); exit(1);}
...
if(!(f=fopen("cork.cld","w"))){printf("Could not open file 'cork.cld'\r\n"); exit(1);}
fprintf(f,"0\r\n");
fclose(f);
